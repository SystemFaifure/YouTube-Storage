...
class VideoEncodeApp(tk.Tk):
    ...
    def _build_settings_tab(self, parent: ttk.Frame) -> None:
        ...
        hint = (
            "Tips: mp4v works for .mp4 on many systems. If writer fails, use codec=MJLS and output extension .avi. "
    ...
    def _browse_encode_output(self) -> None:
        path = filedialog.asksaveasfilename(
            ...
            filetypes=[("All files", "*.*"), ("AVI", "*.avi"), ("MP4", "*.mp4")],
        )
        if path:
    ...
    def _browse_decode_input(self) -> None:
        path = filedialog.askopenfilename(
            ...
            filetypes=[("All files", "*.*"), ("Video files", "*.mp4 *.avi *.mov *.mkv *.webm")],
        )
        if path:
    ...
    def _config(self) -> CodecConfig:
        codec = self.codec_var.get().strip() or "MJLS"
        if len(codec) < 4:
            codec = (codec + "MJLS")[:4]
        codec = codec[:4]
    ...
    def _apply_high_accuracy_preset(self) -> None:
        ...
        self._log("Applied high-accuracy preset: codec=FFV1, pixel_size=8, tolerance=30, output=.avi")
